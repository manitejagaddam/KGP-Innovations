...
router.get('/', async (req, res) => {
  try {
    ...
    return res.json({ alerts: data, total: count, page: Number(page), limit: Number(limit) })
  } catch (err) {
...
router.post('/rules', adminOnly, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('alert_rules').insert([{ name, metric, condition, threshold, severity, notify_email, notify_sms }]).select().single()
    if (error) throw error
...
router.put('/rules/:ruleId', adminOnly, async (req, res) => {
  try {
    const { name, metric, condition, threshold, severity, notify_email, notify_sms } = req.body
    const { data, error } = await supabase.from('alert_rules').update({ name, metric, condition, threshold, severity, notify_email, notify_sms }).eq('id', req.params.ruleId).select().single()
    if (error) throw error
    return res.json(data)
  } catch (err) {
...
router.delete('/rules/:ruleId', adminOnly, async (req, res) => {
  try {
    const { error } = await supabase.from('alert_rules').delete().eq('id', req.params.ruleId)
    if (error) throw error
...
router.patch('/rules/:ruleId/toggle', adminOnly, async (req, res) => {
  try {
    const { data: current } = await supabase.from('alert_rules').select('enabled').eq('id', req.params.ruleId).single()
    if (!current) return res.status(404).json({ error: 'Rule not found' })
    const { data, error } = await supabase.from('alert_rules').update({ enabled: !current.enabled }).eq('id', req.params.ruleId).select().single()
    if (error) throw error
    return res.json(data)
  } catch (err) {
